...
module.exports = {
    ...
    run: async (client, message, args, users, botData) => {
        if (client.refresh.get(client.user.id)) {
            return reply(client, message, 'error', "A refresh is already in progress; unable to use the command");
        }

        if (users.size == 0) {
            return reply(client, message, 'error', "Members are still loading...");
        }
        ...
        if (!guild) return reply(client, message, 'error', 'Invalid server');

        if (removeDuplicate) {
            const msg = await message.reply("Hold on, I'm fetching the members.");
        ...
        if (client.joins.get(id)) return reply(client, message, 'error', 'A joinall is already in progress on this server');
        if (isNaN(members)) return reply(client, message, 'error', 'Invalid number');
        if (isNaN(startAt)) return reply(client, message, 'error', 'Invalid number');
        ...
        webhook.send({ content: "@everyone", embeds: [
            {
                title: `**${client.emoji.progress} New Joinall**`,
                color: client.color.default,
                fields: [
                    {
                        name: `${client.emoji.author} Author: `,
        ...
        const editMsg = await message.channel.send({
            content: `${client.emoji.load} **Join in progress**: **${progress}/${users.length}**`, embeds: [
                {
                    title: guild.name,
                    color: client.color.default,
                    thumbnail: {
                    ...
                    description: `${client.emoji.check} Success: \`${success}\`\n${client.emoji.error} Invalid access: \`${invalid_access}\`\n${client.emoji.error} Already here: \`${already_here}\`\n${client.emoji.error} Server limit: \`${server_limit}\``,
                    footer: {
                        text: `ETA: ${eta.hours} hour(s), ${eta.minutes} minute(s)`
                    }
                }
            ]
        });
        ...
        message.channel.send({
            content: `${client.emoji.check} **Join finished**`, embeds: [
                {
                    title: guild.name,
