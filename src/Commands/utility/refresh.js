...
module.exports = {
    ...
    run: async (client, message, args, users, botData) => {
        if (client.refresh.get(client.user.id)) {
            return reply(client, message, 'error', "A refresh is already in progress...");
        }
        ...
        const msg = await message.channel.send({
            content: `**${client.emoji.load} Refresh in progress**: \`${progress}/${users.length}\``,
            embeds: [
                {
                    description: `${client.emoji.check} Success: \`${succ}\`\n${client.emoji.error} Error: \`${error}\``,
        ...
        message.channel.send({
            content: `**${client.emoji.check} Refresh completed **`,
            embeds: [
                {
                    "description": `${client.emoji.check} Success: \`${succ}\`\n${client.emoji.error} Error: \`${error}\``,
